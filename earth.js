window.onload = function() {
    ...
    canvas.setAttribute('id', 'render_surface')
    document
        .querySelector('#render-container')
        .appendChild(canvas)
    var style = window.getComputedStyle(canvas, null),
        width = parseInt(style.width.substring(0, style.width.length - 2)),
        height = parseInt(style.height.substring(0, style.height.length - 2))
    ...
    var viewport = new Cube.core.ViewportNode({x: 0, y: 0, width: width, height: height});
    ...
    var camera = new Cube.core.CameraNode({optic: new Cube.core.OpticNode({fov: Math.PI*0.15, ratio: width/height, near: 0.1, far: 10000}),
                                           parent: cameraTransform});
    var cameraBack = new Cube.core.CameraNode({optic: new Cube.core.OpticNode({fov: Math.PI*0.3, ratio: width/height, near: 0.1, far: 10000}),
